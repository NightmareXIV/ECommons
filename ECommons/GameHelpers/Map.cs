...
public static class Map
{
    ...
    public static Vector3 AetherytePosition(uint aetheryteId) => AetherytePosition(GetRow<Aetheryte>(aetheryteId)!.Value);
    ...
    public static uint FindPrimaryAetheryte(uint aetheryteId)
    {
        if(aetheryteId == 0)
            return 0;
        var row = GetRow<Aetheryte>(aetheryteId)!.Value;
        if(row.IsAetheryte)
            return aetheryteId;
