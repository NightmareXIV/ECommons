...
public static class EzIPC
{
		...
		private static readonly List<EzIPCDisposalToken> Unregister = [];
		private static readonly Type[] FuncTypes = [typeof(Func<>), typeof(Func<,>), typeof(Func<,,>), typeof(Func<,,,>), typeof(Func<,,,,>), typeof(Func<,,,,,>), typeof(Func<,,,,,,>), typeof(Func<,,,,,,,>), typeof(Func<,,,,,,,,>), typeof(Func<,,,,,,,,,>)];
		private static readonly Type[] ActionTypes = [typeof(Action<>), typeof(Action<,>), typeof(Action<,,>), typeof(Action<,,,>), typeof(Action<,,,,>), typeof(Action<,,,,,>), typeof(Action<,,,,,,>), typeof(Action<,,,,,,,>), typeof(Action<,,,,,,,,>), typeof(Action<,,,,,,,,,>)];
				...
				foreach (var reference in instanceType.GetFieldPropertyUnions(bFlags))
				{
						try
						{
								...
								if (attr != null)
								{
										...
										ipcName = ipcName.Replace("%p", Svc.PluginInterface.InternalName);
										var isNonGenericAction = reference.UnionType.IsAssignableTo(typeof(Action));
										if (isNonGenericAction || ((Type[])[.. FuncTypes, .. ActionTypes]).Any(t => t.IsAssignableFrom(reference.UnionType.GetGenericTypeDefinition())))
										{
												...
												var isAction = isNonGenericAction || ActionTypes.Any(t => t.IsAssignableFrom(reference.UnionType.GetGenericTypeDefinition()));
				...
				foreach (var reference in instanceType.GetFieldPropertyUnions(bFlags))
				{
						try
						{
								...
								if (attr != null)
								{
										...
										ipcName = ipcName.Replace("%p", Svc.PluginInterface.InternalName);
										var isNonGenericAction = reference.UnionType.IsAssignableTo(typeof(Action));
										if (isNonGenericAction || ActionTypes.Any(t => t.IsAssignableFrom(reference.UnionType.GetGenericTypeDefinition())))
										{
