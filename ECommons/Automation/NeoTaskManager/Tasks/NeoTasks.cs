...
public static unsafe class NeoTasks
{
    ...
    {
        return new(() => Player.Interactable && !Player.IsAnimationLocked && GenericHelpers.IsScreenReady(), "Wait for screen fadeout complete", new TaskManagerConfiguration(timeLimitMS: (int)TimeSpan.FromMinutes(10).TotalMilliseconds).With(configuration));
    }

    public static TaskManagerTask WaitForNotOccupied(TaskManagerConfiguration? configuration = null)
    {
        return new(() => !GenericHelpers.IsOccupied() && Player.Interactable && !Player.IsAnimationLocked && GenericHelpers.IsScreenReady(), "Wait for screen fadeout complete", new TaskManagerConfiguration(timeLimitMS: (int)TimeSpan.FromMinutes(10).TotalMilliseconds).With(configuration));
    }
}
