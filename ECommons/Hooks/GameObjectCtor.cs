...
{
    public const string Sig = "48 8D 05 ?? ?? ?? ?? C7 81 ?? ?? ?? ?? ?? ?? ?? ?? 48 89 01 48 8B C1 C3";
    ...
    private static Hook<GameObjectConstructorDelegate> GameObjectConstructorHook = null;

    private static event GameObjectConstructorCallbackDelegate _gameObjectConstructorEvent;
    /// <summary>
    ///     Add a <see cref="GameObjectConstructorCallbackDelegate"/> subscriber
    ///     to this event be called when a Game Object is created.
    /// </summary>
    public static event GameObjectConstructorCallbackDelegate GameObjectConstructorEvent
    {
        add
        {
            Hook();
    ...
    private static void Hook()
    {
        ...
        {
            GameObjectConstructorHook = Svc.Hook.HookFromAddress<GameObjectConstructorDelegate>(ptr, GameObjectConstructorDetour);
            Enable();
            PluginLog.Information("Requested Game Object ctor() hook and successfully initialized");
        }
        else
        {
